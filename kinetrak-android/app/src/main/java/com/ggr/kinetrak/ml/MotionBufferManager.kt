...
class MotionBufferManager(
    ...
    /**
     * Checks if motion buffering is currently active.
     */
    fun isBuffering(): Boolean = isRecording.get()

    /**
     * Stops buffering, prepares the flattened 45x6 tensor buffer, and classifies the gesture
     * asynchronously using SnapdragonNPU with a 1500ms timeout guard, falling back to
     * HeuristicGestureFallback upon timeout or exception.
     *
     * Latches the resulting action into BridgeState.pendingAction and sets
